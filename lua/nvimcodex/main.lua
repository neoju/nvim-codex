...
local state = require("nvimcodex.state")
...
function main.toggle(scope)
    if state.get_enabled(state) then
        log.debug(scope, "nvimcodex is now disabled!")

        return main.disable(scope)
    end

    log.debug(scope, "nvimcodex is now enabled!")
...
function main.enable(scope)
    if state.get_enabled(state) then
        log.debug(scope, "nvimcodex is already enabled")
    ...
    -- saves the state globally to `_G.Nvimcodex.state`
    state.save(state)
end
...
function main.disable(scope)
    if not state.get_enabled(state) then
        log.debug(scope, "nvimcodex is already disabled")
